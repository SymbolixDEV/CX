...
public:
    ...
    void run() OVERRIDE
    {
        ...
        TC_LOG_INFO(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Starting up anti-freeze thread (%u seconds max stuck time)...", _delaytime/1000);
        _loops = 0;
        _lastChange = 0;
        while (!World::IsStopped())
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: World Thread hangs, kicking out server!");
                ASSERT(false);
            }
        }
        TC_LOG_INFO(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Anti-freeze thread exiting without problems.");
    }
};
...
int Master::Run()
{
    ...
    if (highPriority)
    {
        if (setpriority(PRIO_PROCESS, 0, PROCESS_HIGH_PRIORITY))
            TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Can't set worldserver process priority class, error: %s", strerror(errno));
        else
            TC_LOG_INFO(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: worldserver process priority class set to %i", getpriority(PRIO_PROCESS, 0));
    }
    ...
    TC_LOG_INFO(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: %s (worldserver-daemon) ready...", _FULLVERSION);
...
bool Master::_StartDB()
{
    ...
    if (dbString.empty())
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: World database not specified in configuration file");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: World database: invalid number of worker threads specified. "
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Cannot connect to world database %s", dbString.c_str());
        return false;
    }
    ...
    if (dbString.empty())
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Character database not specified in configuration file");
        return false;
    }
    ...
    if (asyncThreads < 1 || asyncThreads > 32)
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Character database: invalid number of worker threads specified. "
            "Please pick a value between 1 and 32.");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Cannot connect to Character database %s", dbString.c_str());
        return false;
    }
    ...
    if (dbString.empty())
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Login database not specified in configuration file");
        return false;
    }
    ...
    if (asyncThreads < 1 || asyncThreads > 32)
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Login database: invalid number of worker threads specified. "
            "Please pick a value between 1 and 32.");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Cannot connect to login database %s", dbString.c_str());
        return false;
    }
    ...
    if (!realmID)
    {
        TC_LOG_ERROR(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Realm ID not defined in configuration file");
        return false;
    }
    TC_LOG_INFO(LOG_FILTER_WORLDSERVER, "WAR-SQUAD: Realm running as realm ID %d", realmID);
